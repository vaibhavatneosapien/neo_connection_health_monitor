...
> **0.4.0 — `serverUnreachable` retired (sanctioned decision, supersedes the sections below).** Server-down ownership moved to the consumer app's firebase `system_banners` channel (Approach A). The package no longer emits `serverUnreachable`: a server probe that fails while the internet is up now returns `healthy` (the `!hasInternet` branch is unchanged → `internetDisconnected`). The enum value and its producing `return` are kept commented for source compatibility and re-enablement (Approach C). **Wherever a section below (§1 enum, §4 disambiguation, §Decisions "MUST NOT be collapsed", required test cases #2/#3/#13/#13b, "four/five-state enum") describes `serverUnreachable` as emitted or as a load-bearing distinct state, read it as pre-0.4.0 history.** Two accepted consequences: server-down is now ops-manual (no banner if ops doesn't publish), and a server fault with internet up — including a wrong `healthPath` (404) — fails silent-`healthy`. Full rationale, blast radius, and the app-side follow-ups (dead `serverUnreachable → serverDown` mapping, retired `NetworkFailureTicker`, and the un-built banner-priority override) are in `docs/plans/2026-07-31-001-refactor-decouple-server-unreachable-firebase-plan.md`.
