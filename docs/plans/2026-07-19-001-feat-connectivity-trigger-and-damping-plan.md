...
- `serverUnreachable` copy. The consumer app renders nothing for it today. It was blocked on design capacity rather than engineering when this plan was written, which is why it does not compete with this work; the design landed on 2026-07-30 (Figma `5838-19485`) and it is tracked in the follow-up milestone.
...
Its one sanctioned path is an optional probe callback on the constructor, which `CLAUDE.md` §Out of scope already permits ("an optional `void Function(Object)` log callback in a future revision if needed, but no framework dependency"). That callback is the concrete prerequisite for revisiting KTD13. See Risks measurement 3 for owner, date, and default outcome.

**Signature: widen past `ConnectionHealthState raw`.** The obvious signature `void Function(ConnectionHealthState raw)` is too narrow, and building it that way forecloses a second consumer for free. It collapses every non-2xx and every transport failure into one `serverUnreachable` — so a `404` from a misconfigured `healthPath`, a `503` from a real outage, and a blocked-by-firewall `SocketException` are all the same value, when they are the exact distinctions `CLAUDE.md` §4 "Known limits" wants surfaced (misconfiguration vs. outage vs. path-blocked). `_runCheck` already computes both discriminators and discards them (`connection_health_monitor.dart:458-469` catches the three exception kinds; `:500` computes the status code). Whatever gets built should carry the raw HTTP status *or* the exception type alongside the derived `ConnectionHealthState`, e.g. `void Function(ConnectionHealthState state, {int? statusCode, Object? error})` — one callback then serves both KTD13's instability measurement AND §4's misconfiguration diagnostic. No behavior change to `stream` or `currentState`; it is a pure observation seam.
...
- **`serverUnreachable` banner copy** — **no longer blocked; the design exists.** Figma node `5838-19485` ("Server down" / "Neo 1 is still capturing. Your memory will sync automatically once the server is back online.") is a fourth variant of the same banner shell, confirmed 2026-07-30. The remaining work is entirely `../neo`: a `serverDown` value on `NetworkBannerStatus`, a fourth `_VariantSpec`, and swapping the `_show(hidden)` in `ConnectionHealthCubit._onState`'s `serverUnreachable` arm — whose `TODO(neo)` still says design has no such banner. No package change: `ConnectionHealthState.serverUnreachable` already emits.
...
4. **The probe-RTT distribution (p50/p95/p99) across the real network mix, tier-2 Indian ISPs included, to calibrate `slowThreshold`.** The 3-second cutoff is a bare default with no field data behind it — unlike `requestTimeout`, it carries no documented rationale. This measurement gates two things: whether 3 s is the right `weakNetwork` boundary at all, and whether an **adaptive/relative threshold** (`SRTT + K·RTTVAR`, the RFC 6298 shape, or a per-network floor) is worth building. **Do not build the adaptive threshold speculatively** — it needs a baseline reset on WiFi↔cellular change, which requires connectivity/lifecycle signals this package deliberately cannot observe (§8), so it either needs the consumer to feed network-change events in or it silently misfires across a network switch. Reserve it exactly like `serverDegraded`: named, understood, built only on this data. The industry norm (Chromium NetworkQualityEstimator, ExoPlayer, hls.js — all use smoothed/percentile estimates, never a single sample against a fixed constant) is **not** a reason to add EWMA/windowed smoothing here: those assume a dense sample rate (per-RTT, per-segment), and at this package's 1–5 min cadence a smoother would hold a 5–15 min stale value — worse than a fresh sample for a live banner. The single-sample verdict is correct for the cadence; the confirmation gate (`downConfirmationCount: 2`, already shipped by the consumer) is the right damping, and is the same N-in-a-row hysteresis latch Envoy/HAProxy use. **Owner:** Vaibhav Pandey. **Review date:** 2026-09-30. **Default if not taken by then:** leave 3 s and the single-sample design as-is; close the adaptive-threshold reservation as won't-fix.
